...
export function AdminTasksSection({
  ...
  const [settingsTab, setSettingsTab] = useState<TaskSettingsTab>('details');
  ...
  const moveTask = (taskId: string, delta: -1 | 1) => {
    const currentIndex = taskIndexById[taskId];
    const targetIndex = currentIndex + delta;
    if (!Number.isFinite(currentIndex) || targetIndex < 0 || targetIndex >= tasks.length) {
      return;
    }
  ...
  const confirmDeleteTask = (task: TaskInfo) => {
    ...
    const confirmed = window.confirm(`${t('taskDeleteConfirm')}\n${taskLabel(task)}`);
    if (!confirmed) {
      return;
    }
  ...
  const taskSettingsModal = editingTask ? (
    <div className="event-modal-backdrop" onClick={closeTaskSettings}>
      <div className="event-modal task-settings-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="task-settings-tabs" role="tablist" aria-label={t('edit')}>
          <button
            type="button"
            role="tab"
            aria-selected={settingsTab === 'details'}
            className={`task-settings-tab ${settingsTab === 'details' ? 'active' : ''}`}
            onClick={() => setSettingsTab('details')}
          >
            {t('taskSettingsTabDetails')}
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={settingsTab === 'pipeline'}
            className={`task-settings-tab ${settingsTab === 'pipeline' ? 'active' : ''}`}
          ...
          {settingsTab === 'details' ? (
            <>
              ...
              <div className="task-modal-actions">
                ...
                {editingTask.deletable ? (
                  <button
                    className="button danger small"
                    type="button"
                    onClick={() => confirmDeleteTask(editingTask)}
                    disabled={isTaskDeleteBusy(editingTask.id)}
                  >
                    {isTaskDeleteBusy(editingTask.id) ? t('loading') : t('taskDelete')}
                  </button>
                ) : null}
              </div>
            </>
            ...
            editingTaskConfig ? (
              <>
                ...
                <label className="checkbox-inline pipeline-field">
                  <input
                    type="checkbox"
                    checked={editingTaskConfig.studentSendEventEnabled}
      ...
      <section className="panel hero panel-animate full-width">
        ...
        <div className="tasks-list">
          {tasks.map((task) => {
            ...
            const index = taskIndexById[task.id] ?? -1;
            const canMoveUp = index > 0;
            const canMoveDown = index >= 0 && index < tasks.length - 1;
            return (
